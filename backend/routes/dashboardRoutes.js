...
router.post("/add-subjects", async (req, res) => {
    ...
    try {
        ...
        pool.query(checkQuery, [teacher_id, subject, semester, branch], (err, results) => {
            if (err) {
            ...
            pool.query(insertQuery, [teacher_id, subject, semester, branch], (err, result) => {
                if (err) {
...
router.get("/attendance/:semester/:branch/:subject", (req, res) => {
    ...
    pool.query(query, [semester, branch, subject], (err, results) => {
        ...
        if (results.length === 0) {
            console.warn("⚠️ No attendance records found for:", { semester, branch, subject });
...
router.post("/attendance/update", (req, res) => {
    ...
    attendanceData.forEach(({ student_id, subject, semester, branch, total_classes, attended_classes }) => {
        pool.query(query, [student_id, subject, semester, branch, total_classes, attended_classes], (err) => {
